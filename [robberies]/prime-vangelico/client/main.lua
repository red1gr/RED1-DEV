                    ...
                    if not isCooldown then
                        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
                            if hasItem then
                                ...
                                TriggerServerEvent('qb-logs:server:createLog', 'vangelico', 'Player Started Vangelico Robbery', "", source)
                                ...
                                function() -- Successfully Disable Cameras
                                    ...
                                    if Config.DoorLock == "nui" then
                                        ...
                                        TriggerServerEvent('qb-doorlock:server:updateState', "vangelicodoor", false, false, false, true)
                ...
                if hasItem then
                    ...
                    function(Success)
                        if Success then
                            ...
                            if itemChance <= 99 then
                                TriggerServerEvent('yonatan:GiveItem')
                            end
                        else
                            SecurityFailed()
                            SecurityFailedAnim()
...
RegisterNetEvent('nc-vangelico:client:rebootsystem', function()
    if Config.Locales == true then
        ...
        QBCore.Functions.Notify("The System Will Reboot In 15 Seconds!", "error", 3500)
    end

    if Config.DoorLock == "nui" then
        Citizen.Wait(15000)
        TriggerServerEvent('qb-doorlock:server:updateState', "vangelicodoor", true, false, false, true)
