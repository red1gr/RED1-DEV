        ...
        if hasItem then
            ...
            Drilling.Start(function(status)
                if status then
                    ...
                    ExtendedSetup()
                    TriggerServerEvent('pacificheist:server:doorSync', index)
                    TriggerServerEvent('pacificheist:server:openVaultGold')
...
RegisterNetEvent('pacificheist:client:openVaultGold')
AddEventHandler('pacificheist:client:openVaultGold', function()
    local FleecaObject = GetClosestObjectOfType(Config.Banks["fleeca"]["coords"]["x"], Config.Banks["fleeca"]["coords"]["y"], Config.Banks["fleeca"]["coords"]["z"], 5.0, Config.Banks["fleeca"]["object"], false, false, false)
    local targetHeading2 = Config.Banks["fleeca"]["heading"].open
    local currentHeading2 = GetEntityHeading(FleecaObject)
    local step2 = 0.1
    local delay2 = 20

    while currentHeading2 ~= targetHeading2 do
        if currentHeading2 < targetHeading2 then
            currentHeading2 = currentHeading2 + step2
        else
            currentHeading2 = currentHeading2 - step2
        end

        SetEntityHeading(FleecaObject, currentHeading2)
        Citizen.Wait(delay2)
    end
end)
...
function Hacking(index)
    QBCore.Functions.TriggerCallback('pacificheist:server:hasItem', function(hasItem, itemLabel)
        if hasItem then
            ...
            Wait(2000)
            TriggerEvent("utk_fingerprint:Start", 4, 6, 10, function(outcome, reason)
