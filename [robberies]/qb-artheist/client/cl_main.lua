                                ...
                                if not ArtHeist['cutting'] then
                                    exports['ps-ui']:VarHack(function(success)
                                        if success then
                                            ...
                                            exports['qb-dispatch']:ArtGalleryRobbery()
                                            QBCore.Functions.Notify("The police have been alerted! Hurry up!", "error")
                                        end
                                    end, 5, 6)
                                else
                                    QBCore.Functions.Notify('You dont have an armed switchblade')
                                end
                            else
                                QBCore.Functions.Notify('You need to have a switchblade equipped.')
                            end
                        end
                    end
                end
...
-- function StartHeist()
--     QBCore.Functions.TriggerCallback('qb-artheist:server:getCops', function(cops)
--         if cops >= 4 then
--     QBCore.Functions.TriggerCallback('artheist:server:checkRobTime', function(time)
--         if time then
--                 if ArtHeist['start'] then QBCore.Functions.Notify('There Is Heist Active Right Now.') return end
--                 TriggerServerEvent('artheist:RemoveCard')
...
--             QBCore.Functions.Notify('Go To Steal The Painting')
--             TriggerServerEvent('qb-logs:server:createLog', 'artrobbery', 'Player Started A Art Heist', "", src)
...
--             end
--         else
--             QBCore.Functions.Notify("No Enough Cops", "error")
--         end
--     end)
-- end
-- end)
    ...
    QBCore.Functions.TriggerCallback('qb-artheist:server:getCops', function(cops)
        if cops >= 0 then
            QBCore.Functions.TriggerCallback('artheist:server:checkRobTime', function(time)
                if time then
                    if ArtHeist['start'] then
                        QBCore.Functions.Notify('There Is Heist Active Right Now.')
                        return
                    end
                    TriggerServerEvent('artheist:RemoveCard')
                    for k, v in pairs(ArtHeist['startPeds']) do
            ...
            for k, v in pairs(Config['ArtHeist']['painting']) do
                ...
                SetEntityRotation(ArtHeist['painting'][k], 0, 0, v['objHeading'], 2, true)
                    ...
                    QBCore.Functions.Notify("Not Enough Cops to Start the Heist", "error")
                end
            end)
        else
            QBCore.Functions.Notify("Not Enough Cops to Start the Heist", "error")
...
function HeistAnimation(sceneId)
    ...
    repeat
        exports['qb-drewui']:showInteraction('[E] Cut Right')
        if IsControlJustPressed(0, 38) then
            scenes[1] = true
            exports["qb-drewui"]:hideInteraction()
        end
        Wait(1)
    until scenes[1] == true
    ...
    repeat
        exports['qb-drewui']:showInteraction('[E] Cut Down')
        if IsControlJustPressed(0, 38) then
            scenes[2] = true
            exports["qb-drewui"]:hideInteraction()
        end
    ...
    repeat
        exports['qb-drewui']:showInteraction('[E] Cut Left')
        if IsControlJustPressed(0, 38) then
            scenes[3] = true
            exports["qb-drewui"]:hideInteraction()
        end
    ...
    if ArtHeist['cut'] == 1 then
        if math.random(1,35) < 30 then
            TriggerServerEvent("artheist:dongle")
            TriggerEvent("QBCore:Notify", 'You Got VangelicoCard', 'success')
            TriggerServerEvent('qb-logs:server:createLog', 'artrobbery', 'Player Got VangelicoCard', "", src)
        end
    end
    if ArtHeist['cut'] == #Config['ArtHeist']['painting'] then
        TriggerServerEvent('artheist:server:syncHeistStart')
        TriggerServerEvent('artheist:server:syncAllPainting')
        QBCore.Functions.Notify('Go Sell Your Ass Painting')
        RemoveBlip(stealBlip)
