...
RegisterNetEvent('lockpicks:UseLockpickstore', function(isAdvanced)
    local currentTime = GetGameTimer() -- זמן הסטרינג הנוכחי (במילישניות)

    if currentTime - lastUseTime >= 10000 then -- בדיקה של שתי דקות (180,000 מילישניות)
        ...
        lastUseTime = currentTime -- עדכון הזמן האחרון שבו נעשה שימוש
    else
        QBCore.Functions.Notify("There is a security cooldown, please wait up to 10 seconds, Or the cash register is empty.", "error")
    end
end)
...
RegisterNUICallback('success', function()
    if currentRegister ~= 0 then
        ...
        local yonatan = math.random(1,100)
        if yonatan < 40 then
            TriggerServerEvent('yonatan:GiveItem:Store')
        else
            print('yonatan')
        end
        --exports['qb-dispatch']:StoreRobbery(camId)
